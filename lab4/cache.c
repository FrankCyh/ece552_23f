...
#define CACHE_BINDEX(cp, blks, i) ((struct cache_blk_t *)(((char *)(blks)) + (i) * (sizeof(struct cache_blk_t) + ((cp)->balloc ? (cp)->bsize * sizeof(byte_t) : 0))))
    ...
    if (cmd == Read) {                                            \
        ...
        case 4:                                                   \
            *((word_t *)p) = CACHE_WORD(&blk->data[0], bofs);     \
            break;                                                \
        default: { /* >= 8, power of two, fits in block */        \
            int words = nbytes >> 2;                              \
            while (words-- > 0) {                                 \
                *((word_t *)p) = CACHE_WORD(&blk->data[0], bofs); \
                p += 4;                                           \
...
static void unlink_htab_ent(struct cache_t *cp, /* cache to update */
...
static void update_way_list(struct cache_set_t *set, /* set contained way chain */
                            struct cache_blk_t *blk, /* block to insert */
...
cache_create(char *name, /* name of the cache */
             ...
             unsigned int (*blk_access_fn)(enum mem_cmd cmd, md_addr_t baddr, int bsize, struct cache_blk_t *blk, tick_t now, int prefetch), unsigned int hit_latency, /* latency in cycles for a hit */
                  ...
                  FILE *stream) /* output stream */
{
    ...
    fprintf(stream, "cache: %s: %d-way, `%s' replacement policy, write-back, %d prefetcher type\n", cp->name, cp->assoc, cp->policy == LRU ? "LRU" : cp->policy == Random ? "Random" : cp->policy == FIFO ? "FIFO" : (abort(), ""), cp->prefetch_type);
}
...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
    // md_addr_t next_line_addr = cp->bsize + addr;
    md_addr_t next_line_addr = CACHE_BADDR(cp, cp->bsize + addr);
    if (cache_probe(cp, next_line_addr) == 0) {
        cache_access(cp, Read, next_line_addr, NULL, cp->bsize, 0, NULL, NULL, 1);
    }
}
struct RPT {
    md_addr_t tag;
...
int num_o = 0;
md_addr_t get_PC();
struct RPT *rpt_table_o;

void create_rpt_o(int num_of_entries) {
    rpt_table_o = (struct RPT *)calloc(num_of_entries, sizeof(struct RPT));
    for (int i = 0; i < num_of_entries; i++) {
        rpt_table_o[i].tag      = 0;
        rpt_table_o[i].state[0] = 1;
        rpt_table_o[i].state[1] = 0;
    }
}
int state(int first, int second) {
    return 1 * second + 2 * first;  // 0 No-pred, 1 transient, 2 init, 3 stable
}

/*struct history {
    md_addr_t PC; // PC address
    md addr_t addr[3]; // Three history access
    int addr_pt; //Pointer
    int confidence; // Confidence Level
}
struct history* history_table;
void create_table(int size) {
    history_table = (struct history*)calloc(size, sizeof(struct history));
    for (int i = 0 ; i < size; i++) {
        history_table[i].confidence = 50;
        history_table[i].addr[0] = 0;
        history_table[i].addr[1] = 0;
        history_table[i].addr[2] = 0;
        history_table[i].PC = 0;
        history_table[i].addr_pt = 0;
    }
}*/
int num_of_op    = 0;
int history_size = 500;
md_addr_t history_t[500];
double bias = 0;
double weight_t[500];
int history_index = 0;
int prediction() {
    ...
    for (int i = 0; i < history_size; i++) {
        sum += weight_t[i] * history_t[i];
    }
    return (sum > 0) ? 1 : 0;
}

void train(int target) {
    int pred  = prediction();
    ...
    if (error != 0) {
        bias += error;
        for (int i = 0; i < history_size; i++) {
            weight_t[i] += error * history_t[i];
        }
    }
}
md_addr_t last_addr = 0;  // Last cycle address
void open_ended_prefetcher_1(struct cache_t *cp, md_addr_t addr) {  // Solution 1,using perceptron model. Performs worse than our solution 2.
    /*int history_table_size = 5000000;
    int index = (int)((get_PC() >> 3) % history_table_size);
    if (history_table[index].PC == get_PC()) {
        if ()
    } else {	// Different recording, do not prefetch.
        history_table[index].confidence = 50;
        history_table[index].addr[0] = 0;
        history_table[index].addr[1] = 0;
        history_table[index].addr[2] = 0;
        history_table[index].PC = get_PC();
        history_table[index].addr_pt = 0;
    }*/
    if (num_of_op < history_size) {
        history_t[history_index] = addr;
        history_index++;
        history_index = history_index % history_size;
        last_addr     = addr;
    } else {
        int pred = prediction();
        if ((last_addr + cp->bsize == addr) && (pred == 1)) {
            history_t[history_index] = addr;
            history_index++;
            history_index = history_index % history_size;
            last_addr     = addr;
...
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {  // Solution 2 using a modified version of Stride Prefetcher. This gives better result than our Solution 1.
    int num_of_entries = 1024;
    if (num == 0) {
        create_rpt_o(num_of_entries);
        num++;
    }

    md_addr_t PC = get_PC();
    int index    = (int)((PC >> 3) % num_of_entries);
    int prefetch = 0;
    if (rpt_table_o[index].tag != PC) {  // Either first time or overwritten
        rpt_table_o[index].tag       = PC;
        rpt_table_o[index].prev_addr = addr;
        rpt_table_o[index].stride    = 0;
        rpt_table_o[index].state[0]  = 1;  // Initial
        rpt_table_o[index].state[1]  = 0;
    } else {
        if (rpt_table_o[index].stride == (addr - rpt_table_o[index].prev_addr)) {
            int st = state(rpt_table_o[index].state[0], rpt_table_o[index].state[1]);
            if (st == 0) {
                rpt_table_o[index].state[0] = 0;
                rpt_table_o[index].state[1] = 1;
            } else {
                rpt_table_o[index].state[0] = 1;
                rpt_table_o[index].state[1] = 1;
                prefetch                    = 1;
            }
            // prefetch = 1;
            ...
            if (st == 3) {
                ...
                rpt_table_o[index].state[0] = 0;
                rpt_table_o[index].state[1] = 1;
                rpt_table_o[index].stride   = addr - rpt_table_o[index].prev_addr;
                prefetch                    = 0;
            } else if (st == 1) {
                rpt_table_o[index].state[0] = 0;
                rpt_table_o[index].state[1] = 0;
    ...
    if (prefetch) {
        md_addr_t prefetch_addr = rpt_table_o[index].prev_addr + rpt_table_o[index].stride;
        // prefetch_addr -= prefetch_addr % cp->bsize;
        prefetch_addr = CACHE_BADDR(cp, prefetch_addr);

        if (!cache_probe(cp, prefetch_addr)) {
            cache_access(cp, Read, prefetch_addr, NULL, cp->bsize, 10, NULL, NULL, 1);
        }
    }
}


struct RPT *rpt_table;

/*int state(int first, int second) {

    return 1 * second + 2 * first; // 0 No-pred, 1 transient, 2 init, 3 stable
}*/
void create_rpt(int num_of_entries) {
    rpt_table = (struct RPT *)calloc(num_of_entries, sizeof(struct RPT));
    for (int i = 0; i < num_of_entries; i++) {
        rpt_table[i].tag      = 0;
        rpt_table[i].state[0] = 1;
        rpt_table[i].state[1] = 0;
    }
}

/* Stride Prefetcher */
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
    int num_of_entries = cp->prefetch_type;
    if (num == 0) {
        create_rpt(num_of_entries);
        num++;
    }
    ...
    int index    = (int)((PC >> 3) % num_of_entries);
    int prefetch = 0;
    if (rpt_table[index].tag != PC) {  // Either first time or overwritten
        rpt_table[index].tag       = PC;
        rpt_table[index].prev_addr = addr;
        rpt_table[index].stride    = 0;
        rpt_table[index].state[0]  = 1;  // Initial
        rpt_table[index].state[1]  = 0;
    } else {
        if (rpt_table[index].stride == (addr - rpt_table[index].prev_addr)) {
            int st = state(rpt_table[index].state[0], rpt_table[index].state[1]);
            if (st == 0) {
                rpt_table[index].state[0] = 0;
                rpt_table[index].state[1] = 1;
            } else {
                rpt_table[index].state[0] = 1;
                rpt_table[index].state[1] = 1;
            }
            prefetch                   = 1;
            rpt_table[index].prev_addr = addr;
            ...
            if (st == 3) {
                ...
                rpt_table[index].state[0] = 0;
                rpt_table[index].state[1] = 1;
                rpt_table[index].stride   = addr - rpt_table[index].prev_addr;
                prefetch                  = 1;
            } else if (st == 1) {
                rpt_table[index].state[0] = 0;
                rpt_table[index].state[1] = 0;
    ...
    if (prefetch) {
        md_addr_t prefetch_addr = rpt_table[index].prev_addr + rpt_table[index].stride;
        prefetch_addr           = CACHE_BADDR(cp, prefetch_addr);
        if (!cache_probe(cp, prefetch_addr)) {
            cache_access(cp, Read, prefetch_addr, NULL, cp->bsize, 10, NULL, NULL, 1);
        }
             ...
             int prefetch) /* 1 if the access is a prefetch, 0 if it is not */
{
    ...
    if (repl->status & CACHE_BLK_VALID) {
        ...
        if (repl->status & CACHE_BLK_DIRTY) {
            ...
            cp->writebacks++;
            lat += cp->blk_access_fn(Write, CACHE_MK_BADDR(cp, repl->tag, set), cp->bsize, repl, now + lat, 0);
        }
    }
        ...
        for (blk = cp->sets[i].way_head; blk; blk = blk->way_next) {
            if (blk->status & CACHE_BLK_VALID) {
                ...
                if (blk->status & CACHE_BLK_DIRTY) {
                    ...
                    cp->writebacks++;
                    lat += cp->blk_access_fn(Write, CACHE_MK_BADDR(cp, blk->tag, i), cp->bsize, blk, now + lat, 0);
                }
            }
        }
    }
                 ...
                 tick_t now) /* time of cache flush */
{
    ...
    if (blk) {
        ...
        if (blk->status & CACHE_BLK_DIRTY) {
            /* write back the invalidated block */
            cp->writebacks++;
            lat += cp->blk_access_fn(Write, CACHE_MK_BADDR(cp, blk->tag, set), cp->bsize, blk, now + lat, 0);
        }
